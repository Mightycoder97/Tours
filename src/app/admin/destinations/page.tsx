...
import { createDestination, deleteDestination } from './actions';
  ...
  return (
    <div className="space-y-6 max-w-4xl mx-auto">
      <div className="flex items-center space-x-4">
        ...
        <div>
          <h1 className="text-3xl font-serif font-bold text-primary">Destinos</h1>
          <p className="text-sm text-text-light mt-1">Administra los destinos disponibles.</p>
        </div>
      </div>

      <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
        <div className="md:col-span-1 bg-white rounded-xl shadow-sm border border-gray-100 p-6 h-fit">
          <h3 className="font-bold text-lg mb-4">Nuevo Destino</h3>
          <form action={async (formData) => {
            ...
            <div>
              ...
              <input name="name" required className="w-full px-3 py-2 bg-gray-50 border border-gray-200 rounded-lg focus:outline-none focus:border-primary" placeholder="Ej: Cusco" />
            </div>
            <div>
              <label className="block text-sm font-medium text-text-main mb-1">Slug</label>
              <input name="slug" required className="w-full px-3 py-2 bg-gray-50 border border-gray-200 rounded-lg focus:outline-none focus:border-primary" placeholder="Ej: cusco" />
            </div>
            ...
            <tbody className="divide-y divide-gray-100 text-sm">
              {(destinations || []).map((dest: any) => (
                <tr key={dest.id} className="hover:bg-gray-50">
                  <td className="px-6 py-4 font-medium text-text-main">{dest.name}</td>
                  <td className="px-6 py-4 text-text-light">{dest.slug}</td>
